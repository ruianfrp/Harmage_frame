...
import com.harmonycloud.bean.milestone.MilestoneView;
...
//			try {
            ...
            if (operation.equals("ADD")) {
                ...
                Set<String> listStatus = milestoneService.selectStatus(project_id);
                ProjectMilestoneStatus projectMilestoneStatus = ProjectMilestoneStatus.getProjectMilestoneStatus("顺利进行");
                if (listStatus.contains("延期中") || listStatus.contains("严重延期")) {
                    projectMilestoneStatus = ProjectMilestoneStatus.getProjectMilestoneStatus("严重延期");
                }
                if (flag != null) {
                    milestoneService.updateProjectStatus(projectMilestoneStatus.getProjectStatus(), projectMilestoneStatus.getProjectSubState(), projectId);
                }
            } else if (operation.equals("DELETE")) {
...
//			}
    ...
    public Message startMilestone(@RequestBody @ApiParam(name = "index\nprojectId\nstartTime\nmilestoneStatus", value = "里程碑信息", required = true) Map<String, Object> map) throws Exception {
        ...
        String flag = "正常进行";
        Set<String> listStatus = milestoneService.selectStatus(projectId.toString());
        ProjectMilestoneStatus projectMilestoneStatus = ProjectMilestoneStatus.getProjectMilestoneStatus("顺利进行");
        if (listStatus.contains("延期中") || listStatus.contains("严重延期")) {
            projectMilestoneStatus = ProjectMilestoneStatus.getProjectMilestoneStatus("严重延期");
        }
...
//		}
        ...
        if (Objects.nonNull(hashMap)) {
            ...
            data.put("total", hashMap.size());
            message.setMessage(200, "问题项目排行榜返回成功", data);
        } else {
            log.error("返回错误");
			message.setMessage(400, "返回错误");
        }
		return message;
    }
}
